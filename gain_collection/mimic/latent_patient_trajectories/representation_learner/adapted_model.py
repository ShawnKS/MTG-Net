...
def get_task_losses(task_class_weights):
    ...
    for t in ('next_timepoint_was_measured',):
        params = {}
        if t in task_class_weights: 
            params['weight'] = task_class_weights[t]
...
class GRUModel(nn.Module):
    ...
    def __init__(
        ...
        if self.bidirectional:
            self.h_0 = torch.zeros(2*num_layers,  1, hidden_dim).float().to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        else:
            self.h_0 = torch.zeros(num_layers,  1, hidden_dim).float().to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
...
#         self.cls = ContinuousBertPreTrainingHeads(config) # modify this to get all of the necessary tasks
    ...
    def forward(
        ...
        # for k in ('disch_24h', 'disch_48h', 'Final Acuity Outcome'):
        ...
        print('encode_time', time.time() - starttime)
        # gru learning
        ...
        task_labels = {
            k: df for k, df in dfs.items() if \
                    ...
                    # k not in ('statics', 'ts', 'ts_mask') and df is not None
        }
        # task label获取, 没有设置过的话默认都是1
        ...
        for t in tasks:
            starttime = time.time()
            a = self.task_heads[t](pooled_output) 
            print('decode time', time.time() - starttime)
        starttime_1 = time.time()
        task_logits = {t: self.task_heads[t](pooled_output) for t in tasks}
        print('decode time total ', time.time() - starttime_1)
        ...
        for task, loss_fn, logits, labels, weight in zip_dicts(
            ...
            if((tuning == True) & (weight < 0) ):
                weight = 0
            weights_sum += abs(weight) # We do it like this so that we only track tasks that are actually used.
            if task in self.task_permits_missingness:
                ...
                try:
                    if task in ('disch_24h', 'disch_48h'):
                        ...
                        # print(logits)
                        # print("labels_smoothed shape", labels_smoothed.shape)
                        # print(labels_smoothed)
                        # 计算loss_fn( logits, labels_smoothed ), logits nan处补零
...
# dfs 是一个 batch
        if 'rolling_ftseq' in dfs:
            weights_sum += abs(self.task_weights['rolling_ftseq'])
        ...
        if 'next_timepoint' and 'next_timepoint_was_measured' in dfs:
            weights_sum += abs(self.task_weights['next_timepoint'])
        ...
        if 'masked_imputation' in self.task_weights and self.task_weights['masked_imputation'] > 0:
            ...
            weights_sum += abs(self.task_weights['masked_imputation'])
