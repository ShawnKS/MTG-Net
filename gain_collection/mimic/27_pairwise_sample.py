...
def get_args():
    ...
    parser.add_argument('--frac_num',type=int,default=50)
    parser.add_argument('--ratio',type=float,default = 0.1)
    args = parser.parse_args()
    return args

tasks = [ 'icd_infection', 'icd_neoplasms', 'icd_endocrine', 'icd_blood', 'icd_mental', 'icd_nervous',
...
pairwise_tasks = []
for i in range(2):
    pairwise_tasks.extend(combine(tasks, i+1))
...
for i in range(globalargs.frac_num):
    ...
    ablate_tasks = [ 'icd_infection', 'icd_neoplasms', 'icd_endocrine', 'icd_blood', 'icd_mental', 'icd_nervous',
         'icd_circulatory', 'icd_respiratory', 'icd_digestive', 'icd_genitourinary', 'icd_pregnancy',
         'icd_skin', 'icd_musculoskeletal', 'icd_congenital', 'icd_ill_defined','icd_injury', 'disch_24h', 'disch_48h', 'mort_24h', 'mort_48h',
         'Long LOS', 'Readmission 30', 'Final Acuity Outcome', 'next_timepoint_was_measured', 'dnr_24h', 'dnr_48h', 'cmo_24h', 'cmo_48h']
    this_run_dir = run_dir + str( (base_num+i) ) + '/'
