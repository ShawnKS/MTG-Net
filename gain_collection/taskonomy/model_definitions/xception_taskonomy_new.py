    ...
    def input_per_task_losses(self,losses):
        ...
        if XceptionTaskonomy.count < 200:
            ...
            with open("gradnorm_weights.txt", "a") as myfile:
                myfile.write(str(XceptionTaskonomy.count)+': '+str(GradNormRepFunction.current_weights)+'\n')
        GradNormRepFunction.current_task_losses=losses

    def forward(self, input):
        encoder_starttime = time.time()
        ...
        print('encode time = ',time.time() - encoder_starttime,'seconds')

        # decoder_starttime = time.time()
        outputs={'rep':rep}
        ...
        if self.ozan:
            ...
            for i,(task,decoder) in enumerate(zip(self.task_to_decoder.keys(),self.decoders)):
                ...
                print(task,' decode time = ',time.time() - decoder_starttime,'seconds')
        return outputs
