...
parser.add_argument('--val_name', default='train_tiny.txt', dest='val_name',
                    help='val data')
parser.add_argument('--train_name', default='val_tiny.txt', dest='train_name',
                    help='train data')
...
def get_average_learning_rate(optimizer,mlr):
    try:
        if(optimizer.learning_rate < mlr):
            optimizer.learning_date = mlr
        return optimizer.learning_rate
    except:
        s = 0
        for param_group in optimizer.param_groups:
            if(param_group['lr'] < mlr):
                param_group['lr'] < mlr
            s+=param_group['lr']
...
class Trainer:
    def __init__(self,train_loader,val_loader,model,optimizer,criteria,args,checkpoint=None):
        ...
        self.loss_keys = ['ss_l', 'norm_l', 'depth_l', 'key_l', 'edge_l']
        self.criteria=criteria
        self.args = args
        # add a minimum learning rate
        self.mlr = args.minimum_learning_rate
        self.fp16=args.fp16
        ...
        self.lr0 = get_average_learning_rate(optimizer,self.mlr)
    ...
    def train(self):
        ...
        for self.epoch in range(self.start_epoch,self.args.epochs):
            current_learning_rate = get_average_learning_rate(self.optimizer,self.mlr)
            if current_learning_rate < self.args.minimum_learning_rate:
                current_learning_rate = self.args.minimum_learning_rate
            # train for one epoch
            ...
            train_TAG = './TAG_train11.npy'
        # valid_path = os.path.join(args.run_dir, 'valid_TA_'+str(args.seed)+'_'+str(args.ratio)+'.npy'  )
            np.save(train_TAG, transference)

    def checkpoint(self, loss):
        is_best = loss < self.best_loss
        self.best_loss = min(loss, self.best_loss)
        path_name = '/home/covpreduser/Blob/Blob/v-xiaosong/projects/taskgrouping/'
        run_dir = path_name + 'seed_' + str(args.seed)+ '_ratio_' + str(args.ratio) +'/'
    ...
    def train_epoch(self):
        ...
        starting_learning_rate=get_average_learning_rate(self.optimizer,self.mlr)
        while True:
            if batch_num ==0:
                end=time.time()
                epoch_start_time2=time.time()
...
# eta
    ...
    def train_batch(self, input, target):
        ...
        for combined_task in self.loss_keys:
            ...
            for c_name, criterion_fun in self.criteria.items():
                if first_loss is None: first_loss = c_name
                transference[combined_task][c_name] = ((1.0 - (criterion_fun(preds, target) / loss_dict[c_name])) /
                self.optimizer.state_dict()['param_groups'][0]['lr']).detach().cpu().numpy()
        ...
        for grad_task in transference:
            for source in transference[grad_task]:
                if 'Loss' in source:
                    continue
                rev_transference[source][grad_task] = transference[grad_task][source]
    ...
    def adjust_learning_rate(self):
        self.lr = self.lr0 * (0.75 ** (self.ticks))
        if(self.lr < self.mlr):
            self.lr = self.mlr
